...
base_esa <- base_esa %>% 
  ...
  mutate(clase_torrado_PSH_f = factor(clase_torrado_PSH,
                                levels = 1:5,
                                labels = c("Directiva-profesional",
                                             "Media rutinaria",
                                             "Pequeña burguesía",
                                             "Obrera calificada",
...
tabla_calviv_cso_pond <- xtabs(
  POND1R_FIN ~ cal_viv + clase_torrado_PSH_f,
  data = base_hogares
)

tabla_calviv_cso_pond <- round(tabla_calviv_cso_pond, 0)
...
tabla_cal_serv_cso_pond <- xtabs(
  POND1R_FIN ~ cal_serv + clase_torrado_PSH_f,
  data = base_hogares
)

tabla_cal_serv_cso_pond <- round(tabla_cal_serv_cso_pond, 0)
...
tabla_hacina_cso_pond <- xtabs(
  POND1R_FIN ~ hacina + clase_torrado_PSH_f,
  data = base_hogares
)

tabla_hacina_cso_pond <- round(tabla_hacina_cso_pond, 0)
...
tabla_allega_cso_pond <- xtabs(
  POND1R_FIN ~ allega + clase_torrado_PSH_f,
  data = base_hogares
)

tabla_allega_cso_pond <- round(tabla_allega_cso_pond, 0)
...
write.xlsx(
  tabla_hacina_cso_pond,
  file = "salidas/tabla_hacina_cso_pond.xlsx",
  overwrite = TRUE
)
...
write.xlsx(
  tabla_cal_serv_cso_pond,
  file = "salidas/tabla_cal_serv_cso_pond.xlsx",
  overwrite = TRUE
)
```
...
tabla_grafico_cal_viv <- base_hogares %>% 
  filter(!is.na(cal_viv) & !is.na(clase_torrado_PSH)) %>% 
  group_by(clase_torrado_PSH, cal_viv) %>% 
  summarise(
    total = sum(POND1R_FIN, na.rm = TRUE),
    .groups = "drop"
  ) %>% 
  group_by(clase_torrado_PSH) %>% 
  mutate(
    porcentaje = total / sum(total) * 100
  )

# Ajustamos etiquetas en dos reglones
tabla_grafico_cal_viv <- tabla_grafico_cal_viv %>% 
mutate(clase_torrado_PSH_f = factor(clase_torrado_PSH,
                    levels = 1:5,
                    labels = c("Directiva-\nprofesional",
                               "Media\nrutinaria",
