...
```{r}
# Eliminamos las  etiquetas de las variables para evitar problemas con las funciones de manipulación de datos
base_esa <- base_esa %>%
  haven::zap_labels()
...
Para calcular los principales índices de movilidad social, necesitaremos exportar la *tabla de movilidad* con sus valores absolutos a un archivo de Excel. Para ello, utilizaremos la función `write.xlsx()` del paquete `xlsx`. Vamos primero a guardar la tabla en un objeto para luego exportarlo.  
...
write.xlsx(tabla, "salidas/tabla.xlsx", sheetName = "Frecuencias absolutas")
...
write.xlsx(f_esperadas, "salidas/tabla.xlsx", sheetName = "Frecuencias esperadas", append = TRUE)
